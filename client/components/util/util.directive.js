...
angular
  ...
  .directive('iconPicker', ($timeout) => {
    return {
      'template' : '<span class="icon-picker" uib-dropdown on-toggle="toggled(open)">\
                      <span uib-dropdown-toggle class="{{icon || \'fa fa-file-o\'}}"></span>\
                      <ul uib-dropdown-menu>\
                        <li>\
                          <input class="form-control text-center" type="text" ng-model="searchText" placeholder="Icon name" ng-click="$event.preventDefault(); $event.stopPropagation();"/>\
                        </li>\
                        <li>\
                          <a class="btn" icon="fa {{icon.code}}" ng-click="select(icon.code)" ng-repeat="icon in icons | filter:searchText"></a>\
                        </li>\
                      </ul>\
                    </span>',
      'restrict' : 'E',
